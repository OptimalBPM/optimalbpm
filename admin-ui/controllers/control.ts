...
export class ControlController {
    ...
    start_process = (process_definition_id : string) => {
        var _process_data : StartProcessMessage = new StartProcessMessage();
    ...
    kill_agent = () => {
        this.$http.post('control/agent_control',{"address": this.currAgent, "command": "stop", "reason": "Testing to stop an Agent."}).
            success((data, status, headers, config) => {
                this.$scope.current_process_history = data;
    ...
    loadAgents = ():ng.IPromise<any> => {
        ...
        return this.$http.post('node/lookup', {"collection": "node", "conditions": {"schemaRef":"bpm://node_agent.json",
    ...
    loadProcesses = ():ng.IPromise<any> => {
        ...
        // See schema/constants.py
        return this.$http.post('node/lookup', {"collection": "node", "conditions": {
